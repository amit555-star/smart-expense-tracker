...
const currencyPairsScroll = document.querySelector('.currency-pairs-scroll');

// Currency pairs to show (all possible pairs)
const currencyCodes = Object.keys(exchangeRates);
let currencyPairs = [];
currencyCodes.forEach(from => {
    currencyCodes.forEach(to => {
        if (from !== to) currencyPairs.push({ from, to });
    });
});
...
let currentAmount = 0;
let activeCurrencySide = 'from'; // 'from' or 'to'
...
function swapCurrencies() {
    try {
        ...
        // Add visual feedback
        addFeedbackAnimation(fromCurrencyDisplay);
        addFeedbackAnimation(toCurrencyDisplay);
...
// Render a single scrollable list for both from/to currency selection
function renderCurrencyPairs() {
    if (!currencyPairsScroll) return;
    currencyPairsScroll.style.display = '';
    currencyPairsScroll.innerHTML = '';
    currencyCodes.forEach((code, idx) => {
        const isSelectedFrom = code === currentFromCurrency;
        const isSelectedTo = code === currentToCurrency;
        const item = document.createElement('div');
        item.className = 'currency-pair-item';
        if (isSelectedFrom) item.classList.add('selected-from');
        if (isSelectedTo) item.classList.add('selected-to');
        item.innerHTML = `${currencyFlags[code]}<br><span style="font-size:13px;">${code}</span>`;
        // Left click: set as from or to based on activeCurrencySide
        item.onclick = (e) => {
            if (activeCurrencySide === 'to') {
                if (code !== currentFromCurrency) {
                    currentToCurrency = code;
                }
            } else {
                if (code !== currentToCurrency) {
        ...
        // Touch long-press for mobile (set as to)
        let touchTimer;
        item.ontouchstart = () => {
        ...
        currencyPairsScroll.appendChild(item);
    });
    // Scroll to selected currency (show 4 at a time)
    const selectedIdx = currencyCodes.findIndex(code => code === currentFromCurrency || code === currentToCurrency);
    if (selectedIdx > 0 && currencyPairsScroll.children[selectedIdx]) {
        currencyPairsScroll.children[selectedIdx].scrollIntoView({ inline: 'center', behavior: 'smooth' });
    }
}
